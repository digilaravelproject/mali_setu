...
@section('title', 'Publish a Blog — Mali Setu')
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card border-0 shadow-sm" style="border-radius: 24px; overflow: hidden; background: #fff;">
                <div class="card-header bg-primary text-white p-4 border-0 d-flex align-items-center gap-3">
                    ...
                    <div>
                        <h4 class="fw-bold mb-0">Publish a Blog Article</h4>
                        <p class="mb-0 text-white-50 small">Publish your stories, community updates, or expert opinions</p>
                    </div>
                </div>
                    ...
                    <form action="{{ route('blogs.store') }}" method="POST" enctype="multipart/form-data" id="blogCreateForm">
                        ...
                        <div class="row g-3 mb-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="title" id="title" class="form-control border-2 @error('title') is-invalid @enderror" placeholder="Enter Your Blog title" value="{{ old('title') }}" style="border-radius: 12px; border-color: #e5cbd6;">
                                @error('title')
                            ...
                            <div class="col-md-6">
                                ...
                                @if(auth()->user()->user_type === 'bloger' && auth()->user()->blogCategory)
                                    <select name="blog_type_disabled" id="blog_type" class="form-select border-2" style="border-radius: 12px; border-color: #e5cbd6; appearance: none; background-color: #f8fafc;" disabled>
                                        <option value="{{ auth()->user()->blogCategory->id }}" selected>{{ auth()->user()->blogCategory->name }}</option>
                                    </select>
                                    <input type="hidden" name="blog_type" value="{{ auth()->user()->blogCategory->id }}">
                                ...
                                @else
                                    <select name="blog_type" id="blog_type" class="form-select border-2 @error('blog_type') is-invalid @enderror" style="border-radius: 12px; border-color: #e5cbd6; appearance: none; background-image: url('data:image/svg+xml;utf8,<svg xmlns=&quot;http://www.w3.org/2000/svg&quot; width=&quot;24&quot; height=&quot;24&quot; viewBox=&quot;0 0 24 24&quot; fill=&quot;none&quot; stroke=&quot;%23ff4757&quot; stroke-width=&quot;2&quot; stroke-linecap=&quot;round&quot; stroke-linejoin=&quot;round&quot;><polyline points=&quot;6 9 12 15 18 9&quot;></polyline></svg>'); background-repeat: no-repeat; background-position: right 15px center; background-size: 16px;">
                                        ...
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->id }}" {{ (int)old('blog_type') === $cat->id ? 'selected' : '' }}>{{ $cat->name }}</option>
                                        @endforeach
                                    </select>
                                    @error('blog_type')
                                        <div class="text-danger small mt-1 fw-semibold" style="color: #c92f54 !important;">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="description" id="description" rows="5" class="form-control border-2 @error('description') is-invalid @enderror" placeholder="Enter Your Blog description" style="border-radius: 12px; resize: vertical; border-color: #e5cbd6;">{{ old('description') }}</textarea>
                        ...
                        <div class="row g-3 mb-4">
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="media-upload-dropzone border-2 border-dashed p-3 text-center rounded-4 position-relative" style="cursor: pointer; border-color: var(--primary) !important; background-color: rgba(var(--primary-rgb), 0.02); transition: all 0.3s ease;">
                                    ...
                                    <div id="previewContainer" class="d-none mt-1 w-100"></div>
                                </div>
                                @error('media')
                                    <div class="text-danger small mt-1 fw-semibold" style="color: #c92f54 !important;">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end mt-4">
                            ...
                            <button type="submit" class="btn btn-primary px-5 py-2 rounded-pill fw-semibold shadow d-flex align-items-center justify-content-center gap-2" id="submitBtn">
                                <span id="btnText"><i class="fa-solid fa-paper-plane me-1"></i>Publish Article</span>
...
function previewMedia(input) {
    ...
    if (files.length > 0) {
        ...
        // Add a grid container for previews
        const grid = document.createElement('div');
        grid.className = 'row g-2 justify-content-center mb-2';
        previewContainer.appendChild(grid);
        
        // Add a clear/remove all button
        const removeAllBtn = document.createElement('button');
        removeAllBtn.type = 'button';
        removeAllBtn.className = 'btn btn-outline-danger btn-sm rounded-pill';
        removeAllBtn.innerHTML = '<i class="fa-solid fa-trash me-1"></i> Remove All Media';
