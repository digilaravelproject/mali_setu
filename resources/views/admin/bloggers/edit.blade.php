...
<div class="content-area">
    ...
    <div class="card border-0 shadow-sm col-lg-12 mx-auto" style="border-radius:16px;">
        <div class="card-header bg-white py-3">
            <h5 class="mb-0 fw-bold text-dark"><i class="fas fa-user-edit me-2 text-primary"></i>Edit Blogger Details</h5>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('admin.bloggers.update', $blogger->id) }}" method="POST" id="edit-blogger-form">
                @csrf
                @method('PUT')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="name" id="name" class="form-control" placeholder="Enter blogger's full name" value="{{ old('name', $blogger->name) }}" required>
                </div>

                <!-- Email Field with AJAX Uniqueness Checking -->
                <div class="mb-3">
                    <label for="email" class="form-label fw-bold text-secondary">Email Address</label>
                    <input type="email" name="email" id="email" class="form-control" placeholder="Enter unique email address" value="{{ old('email', $blogger->email) }}" required autocomplete="off">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="phone" id="phone" class="form-control" placeholder="Enter phone number" value="{{ old('phone', $blogger->phone) }}" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <select name="blog_category_id" id="blog_category_id" class="form-select @error('blog_category_id') is-invalid @enderror" required>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('blog_category_id', $blogger->blog_category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                        @endforeach
                    </select>
                ...
                <!-- Status Field -->
                <div class="mb-4">
                    <label for="status" class="form-label fw-bold text-secondary">Account Status</label>
                    <select name="status" id="status" class="form-select" required>
                        <option value="active" {{ old('status', $blogger->status) === 'active' ? 'selected' : '' }}>Active (Access Enabled)</option>
                        <option value="suspended" {{ old('status', $blogger->status) === 'suspended' ? 'selected' : '' }}>Suspended (Access Revoked)</option>
                    </select>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const form = document.getElementById('edit-blogger-form');
    ...
    function checkEmailUniqueness(email) {
        ...
        .then(data => {
            if (data.exists) {
                showInvalidState("This email is already in use by another user.");
            } else {
                showValidState();
            }
        })
